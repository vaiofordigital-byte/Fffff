...
const serverSchema = z.object({
  ...
  DATABASE_USER: z.string().min(1).default("evelia"),
  DATABASE_PASSWORD: z.string().default(""),
  DATABASE_NAME: z.string().min(1).default("evelia"),
  DATABASE_CONNECTION_LIMIT: z.coerce.number().int().min(1).max(50).default(10),
  SESSION_COOKIE_NAME: z.string().default("evelia_session"),
  ...
  SMTP_FROM: z.string().default("EVELIA <no-reply@example.com>"),
});
...
export const publicEnv = {
  ...
  appName: process.env.APP_NAME ?? "EVELIA",
};
